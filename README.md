前言：Android开发在eclipse时代，想要开发NDK项目或生成so库，是非常蛋疼的，需要踩坑无数，方能立地成so库；而如今Android Studio时代，开发jni C/C++项目，通过gradel的集成工具，那是一个爽。下面将会介绍两种利用AS和gradel开发NDK项目及生成so库的方式。
## 一.环境准备
...
执行Build->Make Project，生成class文件，class文件的生成路径为： app_path/build/intermediates/classes/debug下的.
